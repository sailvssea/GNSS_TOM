...
#ifndef GPSTOM_MISCMATH_HPP
#define GPSTOM_MISCMATH_HPP
...
namespace gpstom
{
   ...
   {
      if(Y.size() < X.size()) {
         GPSTOM_THROW(Exception("Input vectors must be of same size"));
      }
      size_t i,j;
      T Yx(0);
      for(i=0; i<X.size(); i++) {
                           ...
                           const T& x, T& err) throw(Exception)
   {
      if(Y.size() < X.size() || X.size() < 4) {
         GPSTOM_THROW(Exception("Input vectors must be of same length, at least 4"));
      }
