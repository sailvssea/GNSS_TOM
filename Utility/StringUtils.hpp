...
#ifndef GPSTOM_STRINGUTILS_HPP
#define GPSTOM_STRINGUTILS_HPP
   ...
   namespace StringUtils
   {
                                                      ...
                                                      bool ignoreEmpty)
      {
         ...
         if(delimiter == '\'' || delimiter == '"')
            GPSTOM_THROW(StringException("Delimiter must not be quote"));
         ...
         while(endPos != std::string::npos && endPos <= aStr.length())
         {
            ...
            if(currentDelimiter != delimiter)
               token = gpstom::StringUtils::strip(token,currentDelimiter);

               // remove whitespace at beginning and end
            if(trimWhitespace) token = gpstom::StringUtils::strip(token);
