...
#ifndef GPSTOM_EXCEPTION_HPP
#define GPSTOM_EXCEPTION_HPP
...
#define FILE_LOCATION gpstom::ExceptionLocation(__FILE__, __FUNCTION__, __LINE__)
#else
#define FILE_LOCATION gpstom::ExceptionLocation(__FILE__, "", __LINE__)
#endif
...
/// @ingroup exceptiongroup
#define GPSTOM_THROW(exc) { exc.addLocation(FILE_LOCATION); exc.terminate(); }
/// A macro for adding location when rethrowing an gpstk::Exception
/// @ingroup exceptiongroup
#define GPSTOM_RETHROW(exc) { exc.addLocation(FILE_LOCATION); exc.terminate(); }
#else
/// A macro for adding location when throwing an gpstk::Exception
/// @ingroup exceptiongroup
#define GPSTOM_THROW(exc)   { exc.addLocation(FILE_LOCATION); throw exc; }
/// A macro for adding location when rethrowing an gpstk::Exception
/// @ingroup exceptiongroup
#define GPSTOM_RETHROW(exc) { exc.addLocation(FILE_LOCATION); throw; }
#endif

/// Provide an "ASSERT" type macro
#define GPSTOM_ASSERT(CONDITION) if (!(CONDITION)) {                     \
      gpstom::AssertionFailure exc("Assertion failed: " #CONDITION);     \
      GPSTOM_THROW(exc);                                                 \
   }
...
public: \
   ...
   child(const child& a): parent(a) {} \
      /** Cast constructor. */ \
   child(const gpstom::Exception& a) : parent(a) {} \
      /** \
   ...
   child(const std::string& a, unsigned long b = 0,\
         gpstom::Exception::Severity c = gpstom::Exception::unrecoverable) \
         : parent(a, b, c) \
   {}\
      /** \
       * Common use constructor. \
