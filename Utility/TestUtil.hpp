...
#define TUDEF(CLASS,METHOD) gpstom::TestUtil testFramework(CLASS, METHOD, __FILE__, __LINE__)
...
namespace gpstom
{
   ...
   public:
      ...
      void assert_equals_fp( const T& expected,
          ...
          */
      template<typename T>
      void assert_equals( const gpstom::Matrix<T>& expected,
                          const gpstom::Matrix<T>& got,
                          int lineNumber,
                          const std::string& testMsg = std::string(),
                          T epsilon = -1);
      template<typename T>
      void assert_equals( const gpstom::Vector<T>& expected,
                          const gpstom::Vector<T>& got,
                          int lineNumber,
                          const std::string& testMsg = std::string(),
                          T epsilon = -1);
        ...
        failCount( 0 )
   {
      ...
      std::string file_sep = gpstom::getFileSep();
   ...
   template<typename T>
   void TestUtil ::
   assert_equals( const gpstom::Matrix<T>& expected,
                  const gpstom::Matrix<T>& got,
                  int lineNumber,
                  const std::string& testMsg,
                  T epsilon )
   ...
   template<typename T>
   void TestUtil ::
   assert_equals( const gpstom::Vector<T>& expected,
                  const gpstom::Vector<T>& got,
                  int lineNumber,
                  const std::string& testMsg,
                  T epsilon )
                  ...
                  std::vector<std::string> ignoreRegex )
   {
      ...
      while( !refStream.eof() )
      {
         ...
         if (!ignoreRegex.empty())
         {
            ...
            {
               if (gpstom::StringUtils::isLike(refLine, ignoreRegex[i]))
               {
                  ignore = true;
                  break;
               }
