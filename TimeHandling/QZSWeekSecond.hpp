...
#ifndef GPSTOM_QZSWEEKSECOND_HPP
#define GPSTOM_QZSWEEKSECOND_HPP
...
namespace gpstom
{
   ...
   public:
      ...
      {
         try {
            using gpstom::StringUtils::formattedPrint;
         ...
         catch(gpstom::StringUtils::StringException& e)
         { GPSTOM_RETHROW(e); }
      }
      ...
      bool setFromInfo( const IdToValue& info )
      {
         ...
         for( IdToValue::const_iterator i = info.begin(); i != info.end(); i++ )
         {
            ...
            {
               case 'V':
                  setEpoch( gpstom::StringUtils::asInt( i->second ) );
                  break;
               case 'h':
                  week = gpstom::StringUtils::asInt( i->second );
                  break;
               case 'i':
                  setModWeek( gpstom::StringUtils::asInt( i->second ) );
                  break;
               case 'w':
                  sow = static_cast<double>(gpstom::StringUtils::asInt(i->second))*SEC_PER_DAY;
                  break;
               case 'g':
                  sow = gpstom::StringUtils::asDouble( i->second );
                  break;
               case 'P':
                  timeSystem = static_cast<TimeSystem>(gpstom::StringUtils::asInt( i->second ));
                  break;
               default:
                     // do nothing
                  break;
