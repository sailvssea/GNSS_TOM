...
namespace gpstom
{
   ...
   std::string CivilTime::printf( const std::string& fmt ) const
   {
      ...
      catch( gpstom::StringUtils::StringException& exc )
      {
         GPSTOM_RETHROW( exc );
      }
   }
   ...
   {
      using gpstom::StringUtils::lowerCase;
   ...
   bool CivilTime::operator<( const CivilTime& right ) const
   {
     /// Any (wildcard) type exception allowed, otherwise must be same time systems
      if ((timeSystem != TimeSystem::Any &&
          ...
          timeSystem != right.timeSystem)
      {
         gpstom::InvalidRequest ir("CommonTime objects not in same time system, cannot be compared");
         GPSTOM_THROW(ir);
      }
