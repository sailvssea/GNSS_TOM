...
#ifndef GPSTOM_TABULAR_SAT_STORE_INCLUDE
#define GPSTOM_TABULAR_SAT_STORE_INCLUDE
   ...
   public:
         ...
         const throw(InvalidRequest)
      {
         try
         {
            ...
            {
               InvalidRequest
                  e("Satellite " + gpstom::StringUtils::asString(sat) +
                    " not found.");
               GPSTOM_THROW(e);
            }
            ...
            if(dtable.size() < 2)
            {
               InvalidRequest e("Inadequate data (size < 2) for satellite " +
                                gpstom::StringUtils::asString(sat) +
                                printTime(ttag,fmt));
               GPSTOM_THROW(e);
            }
               ...
               if(--it1 == dtable.begin() && !last)
               {
                  InvalidRequest
                     e("Inadequate data before(3) requested time for"
                       " satellite " + gpstom::StringUtils::asString(sat) +
                       printTime(ttag,fmt));
                  GPSTOM_THROW(e);
               }
               ...
               for(it=tables.begin(); it!=tables.end(); it++)
               {
                  ...
                  for(jt=it->second.begin(); jt!=it->second.end(); jt++)
                  {
                     os << " " << printTime(jt->first,fmt)
                        << " " << gpstom::StringUtils::asString(it->first)
