...
namespace gpstom
{
   ...
   bool BDSEphemeris::isValid(const CommonTime& ct) const
   {
      ...
      catch(Exception& e) { GPSTOM_RETHROW(e); }
   }
   ...
   {
      if(!dataLoadedFlag)
         GPSTOM_THROW(InvalidRequest("Data not loaded"));
     ...
     gpstom::Matrix<double> inertialPos(3,1);
      inertialPos(0,0) = xGK;
      inertialPos(1,0) = yGK;
      inertialPos(2,0) = zGK;

     gpstom::Matrix<double> result(3,1);
     ...
     gpstom::Matrix<double> dmatZ(3,3);
      // Row,Col
     ...
     gpstom::Matrix<double> dIntPos(3,1);
      ...
      cout << "dmatZ : " << dmatZ(2,0) 
                   ...
                   << ", " << dmatZ(2,2) << endl; 
      */
     gpstom::Matrix<double> vresult(3,1);
      vresult =  matZ * matX * dIntPos + 
                dmatZ * matX * inertialPos;

      /* debug
     gpstom::Matrix<double> firstHalf(3,1);
      firstHalf = matZ * matX * dIntPos;
     gpstom::Matrix<double> secondHalf(3,1);
