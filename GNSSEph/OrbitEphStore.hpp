 ...
 * OrbitEph> >, initial and final times and search methods.  Note this
 * class may be used as is, using OrbitEph rather than its
 * descendants, without health or accuracy information; however most
 * likely the user will define a class that inherits this and uses one
 * the classes derived from OrbitEph, for example GPSEphemerisStore
 * and GPSEphemeris. */

#ifndef GPSTOM_ORBITEPHSTORE_HPP
#define GPSTOM_ORBITEPHSTORE_HPP
...
namespace gpstom
{
       ...
       * velocity, and clock data. Also defines a simple interface to
       * remove data that has been added. */
   ...
   public:
          ...
          * single time system, member timeSystem could be defined to
      ...
      virtual Xvt getXvt(const SatID& id, const CommonTime& t) const;
          ...
          *  2: For each satellite: number of entries and time limits,
          *     followed by one line summary per ephemeris with Toe,
          *     Toc, Key, and hours of validity
          *  3: For each satellite: number of entries and time limits,
          *     followed by one line summary per ephemeris with begin,
          *     Toe, Toc, and end valid times
          *  other: For each satellite: number of entries and time
          *     limits, followed by full dump using the
          *     OrbitEph->dump() function.
      ...
      virtual bool isPresent(const SatID& sat) const
      {
         ...
         /** Return the time system of the store. This may be
          * redefined in the default constructor by the derived
          * class. */
      virtual TimeSystem getTimeSystem(void) const
      { return timeSystem; }

         //---------------------------------------------------------------
         // This ends the XvtStore<SatID> interface. Below are
         // interfaces that are unique to this class (i.e. not in the
         // parent class XvtStore<SatID>)
         // ---------------------------------------------------------------

         /** Get the number of OrbitEph objects in this collection for
          * all satellites.
          * @return the number of OrbitEph records in the map for all
          *   satellites. */
      unsigned size(void) const;

         /** Get the number of OrbitEph objects in this store for the
          * given satellite, or if sat.id == -1, the total for all
          * satellites in this system.
          * @return the number of OrbitEph records stored for the
          *   given satellite. */
      ...
      bool isPresent(const SatID& sat, const CommonTime& t) const
      {
          ...
          * user following the regardless of health status. It is
          * suggested that the user call rationalize() to adjust the
          * begin/end times of validity after loading all the elements
          * and before using the store. There exists a second find
          * fuction, findNearOrbitEph() that is provided for
          * compatibility with past uses of the GPSEphemerisStore
          * class. findNearOrbitEph() MAY return elements that are
          * outside the range of validity and therefore need to be
          * used with caution. Therefore, use findNearOrbitEph() only
          * directly, carefully examining the resulting set of orbital
          * elements and make an informed decision before using the
          * OrbitEph.getXvt() functions.
          * @param sat SatID of satellite of interest
          * @param t time with which to search for OrbitEph
          * @return a pointer to the desired OrbitEph, or NULL if no
          *   OrbitEph found. */
      virtual const OrbitEph* findUserOrbitEph(const SatID& sat,
                                               const CommonTime& t)
         const;

         /** Find an OrbitEph for the indicated satellite at time
          * t. The OrbitEph chosen is the one with HOW time closest to
          * the time t, (i.e. with smallest fabs(t-HOW), but still
          * within the fit interval.
          * @param sat the satellite of interest
          * @param t the time of interest
          * @return a pointer to the desired OrbitEph, or NULL if no
          *   OrbitEph found. */
      virtual const OrbitEph* findNearOrbitEph(const SatID& sat,
                                               const CommonTime& t)
         const;

         /** Find an OrbitEph for the indicated satellite at time t,
          * using the find...() routine appropriate for the current
          * search method.
          * @param sat the satellite of interest
          * @param t the time of interest
          * @return a pointer to the desired OrbitEph, or NULL if no
      ...
      bool getSatHealth(const SatID& sat, const CommonTime& t) const
      {
          ...
          * single satellite.  The key is the beginning of the period
          * of validity for each set of elements. */
      typedef std::map<CommonTime, OrbitEph*> TimeOrbitEphTable;

         /** This map holds all unique OrbitEph for each satellite The
          * key is the SatID of the satellite. */
      typedef std::map<SatID, TimeOrbitEphTable> SatTableMap;

         /** Returns a map of the ephemerides available for the
          * specified satellite.  Note that the return is specifically
          * chosen as a const reference.  The intent is to provide
          * "read only" access for analysis.  If the map needs to be
          * modified, see other methods.
          * @param sat SatID of satellite of interest
   ...
   protected:
         ...
         /** The map <SatID, <CommonTime, OrbitEph> > where all
          * OrbitEph are stored. */
...
#endif // GPSTOM_ORBITEPHSTORE_HPP
