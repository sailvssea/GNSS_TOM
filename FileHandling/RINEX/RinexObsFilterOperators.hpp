...
#ifndef GPSTOM_RINEXOBSFILTEROPERATORS_HPP
#define GPSTOM_RINEXOBSFILTEROPERATORS_HPP
...
namespace gpstom
{
   ...
   struct RinexObsDataOperatorLessThanFull :
      public std::binary_function<gpstom::RinexObsData,
                                  gpstom::RinexObsData, bool>
   {
   public:
      ...
      (const std::set<gpstom::RinexObsType>& rohset)
            : obsSet(rohset)
      {}

      bool operator()(const gpstom::RinexObsData& l,
                      const gpstom::RinexObsData& r) const
      {
         ...
         if (l.obs.size() > r.obs.size())
            ...
            // shared fields
         gpstom::RinexObsData::RinexSatMap::const_iterator lItr =
            l.obs.begin(), rItr;
         ...
         while (lItr != l.obs.end())
         {
            ...
            gpstom::RinexObsData::RinexObsTypeMap
               lObs = (*lItr).second,
               rObs = (*rItr).second;

            std::set<gpstom::RinexObsType>::const_iterator obsItr =
               obsSet.begin();

            while (obsItr != obsSet.end())
            {
               gpstom::RinexDatum lData, rData;
               lData = lObs[*obsItr];
               rData = rObs[*obsItr];
   ...
   private:
      std::set<gpstom::RinexObsType> obsSet;
   };
   ...
   struct RinexObsHeaderTouchHeaderMerge :
      public std::unary_function<gpstom::RinexObsHeader, bool>
   {
   public:
      ...
      bool operator()(const gpstom::RinexObsHeader& l)
      {
         if (firstHeader)
         {
            theHeader = l;
         ...
         {
            std::set<gpstom::RinexObsType> thisObsSet,
               tempObsSet;
      ...
      bool firstHeader;
      gpstom::RinexObsHeader theHeader;
      std::set<gpstom::RinexObsType> obsSet;
   };
