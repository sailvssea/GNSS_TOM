...
namespace gpstom
{
      ...
      throw(std::exception, FFStreamError, StringException)
   {
      ...
      while (! (valid & endValid))
      {
         ...
         {
            for(int i = 0; i < 4; i++)
               ionAlpha[i] = gpstom::StringUtils::for2doub(line.substr(2 + 12 * i,12));
            valid |= ionAlphaValid;
         }
         else if (thisLabel == ionBetaString)
         {
            for(int i = 0; i < 4; i++)
               ionBeta[i] = gpstom::StringUtils::for2doub(line.substr(2 + 12 * i,12));
            valid |= ionBetaValid;
         }
         else if (thisLabel == deltaUTCString)
         {
            A0 = gpstom::StringUtils::for2doub(line.substr(3,19));
            A1 = gpstom::StringUtils::for2doub(line.substr(22,19));
