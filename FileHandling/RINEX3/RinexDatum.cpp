...
namespace gpstom
{
   ...
   {
      std::string rv;
      using gpstom::StringUtils::rightJustify;

      if (!dataBlank)
      {
            // double 14.3
         rv += rightJustify(gpstom::StringUtils::asString(data, 3), 14);
      }
      else
      {
         rv += std::string(14, ' ');
      ...
      if ((lli != 0) || !lliBlank)
      {
         rv += rightJustify(gpstom::StringUtils::asString<short>(lli),1);
      }
      else
      {
         rv += " ";
      ...
      if ((ssi != 0) || !ssiBlank)
      {
         rv += rightJustify(gpstom::StringUtils::asString<short>(ssi),1);
      }
      else
      {
         rv += " ";
