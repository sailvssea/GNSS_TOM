...
public:
      ...
      //
   int doReadWriteTests(gpstom::SP3Header::Version ver, const std::string& inFile)
   {
      gpstom::TestUtil  tester( "SP3Data", "Read/Write (" + inFile + ")", __FILE__, __LINE__ );

      std::string  dataFilePath = gpstom::getPathData();
      std::string  dataFileName = dataFilePath + gpstom::getFileSep() +
         "test_input_" + inFile + ".sp3";
      gpstom::SP3Stream  inStream(dataFileName.c_str(), std::ios::in);
      ...
      catch (...)
      {
         ...
         // read in all records
      std::vector<gpstom::SP3Data>  data;
      while (inStream.good() && (EOF != inStream.peek() ) )
      {
         gpstom::SP3Data  record;
         try
         {
            inStream >> record;
      ...
      std::string  tempFileName = tempFilePath + gpstom::getFileSep() +
         "test_output_" + inFile + "_tmp.sp3";
      gpstom::SP3Stream  outStream(tempFileName.c_str(), std::ios::out);
      ...
      catch (...)
      {
         ...
         // write all records
      std::vector<gpstom::SP3Data>::iterator  recordIter = data.begin();
...
int main()  //Main function to initialize and run all tests above
{
   ...
   try
   {
      errorTotal += testClass.doReadWriteTests(gpstom::SP3Header::SP3a, "SP3a");
      errorTotal += testClass.doReadWriteTests(gpstom::SP3Header::SP3a, "SP3ae");
      errorTotal += testClass.doReadWriteTests(gpstom::SP3Header::SP3b, "SP3b");
      errorTotal += testClass.doReadWriteTests(gpstom::SP3Header::SP3c, "SP3c");
      ...
      errorTotal += testClass.doReadWriteTests(gpstom::SP3Header::SP3c, "SP3c_mgex1");
      errorTotal += testClass.doReadWriteTests(gpstom::SP3Header::SP3c, "SP3c_mgex2");
      errorTotal += testClass.doReadWriteTests(gpstom::SP3Header::SP3c, "SP3c_mgex3");
      errorTotal += testClass.doReadWriteTests(gpstom::SP3Header::SP3c, "SP3c_mgex4");
      errorTotal += testClass.doReadWriteTests(gpstom::SP3Header::SP3c, "SP3c_mgex5");
      errorTotal += testClass.doReadWriteTests(gpstom::SP3Header::SP3c, "SP3c_mgex6");
   }
   catch (gpstom::Exception& e)
   {
      ++errorTotal;
